...
Checkpoint / evaluate.py compatibility
---------------------------------------
...
so evaluate.py's UNMODIFIED `PrimitiveHeads.build(device=device,
**head_config)` + `load_state_dict(...)` reconstructs it exactly, and its
three-branch scoring runs unchanged (all three branches score the same
image embedding against three different, differently-soft-prompted text
banks). The learned soft prompt itself is saved separately under
`soft_prompt` / `soft_prompt_config`, since it has no place in
PrimitiveHeads. To evaluate with the trained prompt (rather than plain
zero-shot CLIP text), set VLJEPA_SOFT_PROMPT_CKPT to this checkpoint's path
before running evaluate.py — see scripts/eval_vanilla_narval.sh, and the
hook in models/clip_encoder.py.
"""
